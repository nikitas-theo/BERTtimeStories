...
class SpanMaskingStrategy:
    ...
    def __call__(self, tokens):
        ...
        n_masked = min(
            (tokens >= self.n_special_tokens).long().sum(dim=0), max(1, n_masked)
        )
        ...
        while n_masked > mask.long().sum() and counter > 0:
            ...
            sub_mask[max(0, offset) : min(mask.size(0) - 1, offset + span_length)] = (
                True
            )
...
class BERTDataset(Dataset):
    ...
    def show_random_item(self):
        inputs, attention_mask, outputs = self.__getitem__(self.randint(0, len(self)))

        print(self.tokenizer.decode(inputs.numpy()), flush=True)
