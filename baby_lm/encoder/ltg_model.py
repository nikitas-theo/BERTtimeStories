...
class LTGBERT(nn.Module):
    ...
    def get_attention_scores(self, input_ids, attention_mask):
        static_embeddings, relative_embedding = self.embedding(input_ids)
        attention_scores = self.transformer.forward_attention_scores(
            static_embeddings,
            attention_mask.unsqueeze(1).unsqueeze(2),
            relative_embedding,
        )
    ...
    def get_second_to_last_hidden_states(self, input_ids, attention_mask):
        static_embeddings, relative_embedding = self.embedding(input_ids)
        hidden_states = self.transformer.forward_second_to_last_hidden_states(
            static_embeddings,
            attention_mask.unsqueeze(1).unsqueeze(2),
            relative_embedding,
        )
...
class Encoder(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask, relative_embedding):
        for layer in self.layers:
            if self.activation_checkpointing:
                hidden_states, _ = checkpoint.checkpoint(
                    layer, hidden_states, attention_mask, relative_embedding
                )
            else:
                hidden_states, _ = layer(
                    hidden_states, attention_mask, relative_embedding
                )
        return hidden_states

    def forward_attention_scores(
        self, hidden_states, attention_mask, relative_embedding
    ):
        ...
        for layer in self.layers:
            if self.activation_checkpointing:
                hidden_states, attention_score = checkpoint.checkpoint(
                    layer, hidden_states, attention_mask, relative_embedding
                )
            else:
                hidden_states, attention_score = layer(
                    hidden_states, attention_mask, relative_embedding
                )
    ...
    def forward_all_hidden_states(
        ...
        attention_scores = torch.zeros(
            hidden_states[0].size(1),
            hidden_states[0].size(0),
            hidden_states[0].size(0),
        ...
        for layer in self.layers:
            if self.activation_checkpointing:
                hidden_state, attention_score = checkpoint.checkpoint(
                    layer, hidden_states[-1], attention_mask, relative_embedding
                )
            else:
                hidden_state, attention_score = layer(
    ...
    def forward_second_to_last_hidden_states(
        self, hidden_states, attention_mask, relative_embedding
    ):
...
class GeGLU(nn.Module):
    def forward(self, x):
        ...
        # had to remove approximate for torch=1.9.1
        if "1.9.1" in current_version:
            x = x * F.gelu(gate)
        else:
...
class Attention(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask, relative_embedding):
        ...
        pos = F.embedding(
            self.position_indices[:query_len, :key_len], pos
        )  # shape: [T, T, 2D]
        ...
        attention_scores.add_(
            torch.einsum("bhqd,qkhd->bhqk", query, key_pos * self.scale)
        )
        attention_scores.add_(
            torch.einsum("bhkd,qkhd->bhqk", key * self.scale, query_pos)
        )
