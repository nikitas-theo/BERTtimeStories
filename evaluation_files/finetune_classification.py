...
class ModelArguments:
    ...
    trust_remote_code: bool = field(
        default=False,
        metadata={
            "help": "Whether to allow external code to be executed when running this script."
        },
    )
...
def main():
    ...
    # do LTG-BERT initialization
    import math

    import torch.nn as nn

    std = math.sqrt(2.0 / (5.0 * model.config.hidden_size))
    nn.init.trunc_normal_(model.head.weight, mean=0.0, std=std, a=-2 * std, b=2 * std)
    std = math.sqrt(2.0 / (5.0 * model.config.hidden_size))
    ...
    if data_args.task_name is not None:
        ...
        # special cases
            ...
            # changing template to the one used by LTG-BERT
            template = "{}[SEP] {}"
        elif (
